...
drawCanvas <- function(input, output, session, canvasObjects) {
  ...
  canvas <- canvas +
    geom_rect(data=canvasWide, mapping=aes(xmin=x, ymin=y, xmax=xmax, ymax=ymax,
                                           fill=type), color='black') +
    geom_text(data=canvasWide, aes(x=x+(xmax-x)/2, y=y+(ymax-y)/2, label=id),
              size=5);
  ...
  canvas <- canvas +
    ...
    scale_y_reverse() +
    scale_x_continuous() +
    coord_fixed(xlim=c(-0.1, maxWidth+.1), ylim=c(-0.1, maxHeight+.1),
                expand=FALSE) +
