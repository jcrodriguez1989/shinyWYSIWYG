...
initializeUi <- function(input, output, session) {
  objects <- getObjects();
  updateSelectInput(session, inputId='selectedObject', choices=objects,
                    selected=objects[[1]]);

  renderFnctns <- getShinyRenderFnctns();
  updateSelectInput(session, inputId='newEventRenderFnctn', choices=
                      c('NA', renderFnctns));
...
getObjects <- function() {
  ...
  shinyOutputs <- shinyFunctions[unlist(lapply(shinyFunctions,
                                               function(fnctn) {
    actFnArgs <- names(as.list(args(fnctn)));
...
loadUi <- function(input, output, session, env) {
  ...
  refreshServerCreatorUi(input, output, session, env$canvasObjects,
                         env$validObjects);
