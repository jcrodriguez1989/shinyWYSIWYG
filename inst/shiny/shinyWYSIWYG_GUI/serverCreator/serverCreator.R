...
refreshServerCreatorUi <- function(input, output, session, canvasObjects,
  ...
  updateSelectInput(session, inputId='newEventOutput', choices=
                      c('NA', outputObjsIds));
...
createGlobalVarModal <- function(input, output, session) {
  modal <- fluidPage(
    ...
    textInput(inputId='newGlobalVarInitVal', label='Initialization',
              placeholder='NA')
  )
  btns <- column(12, align='center',
...
delGlobalVar <- function(input, output, session, globalVars) {
  ...
  if (actVar == '') return(globalVars)

  globalVars <- globalVars[!unlist(lapply(names(globalVars),
                                          function(actGlob) {
...
delEvent <- function(input, output, session, events) {
  ...
  if (is.null(selEvent)) return(events);
...
fillEventData <- function(input, output, session, eventData) {
  ...
  updateSelectInput(session, inputId='newEventOutput', selected=
                      eventData$output);
  updateSelectInput(session, inputId='newEventRenderFnctn', selected=
                      eventData$render);
}
